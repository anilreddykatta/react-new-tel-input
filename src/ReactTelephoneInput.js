...
var ReactTelephoneInput = React.createClass({
    ...
    componentWillReceiveProps(nextProps) {
        var nextInputNumber = nextProps.initialValue || nextProps.value || '';
        var inputNumber = this.props.initialValue || this.props.value || '';
        if (nextInputNumber !== inputNumber) {
            var selectedCountryGuess = this.guessSelectedCountry(nextInputNumber.replace(/\D/g, ''));
            var selectedCountryGuessIndex = findIndex(allCountries, selectedCountryGuess);
            var formattedNumber = this.formatNumber(nextInputNumber.replace(/\D/g, ''), selectedCountryGuess ? selectedCountryGuess.format : null);
            var preferredCountries = [];
